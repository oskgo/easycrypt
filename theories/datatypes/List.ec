...
proof. by case: s=> //= s; smt(size_ge0). qed.
...
move=> eq_szS h; have eq_sz: size s1 = size s2 by smt().
have := h 0 => /= ->; first smt(size_ge0).
rewrite (IHs1 s2) // => i le_i_s1.
by move: (h (i + 1)); smt().
qed.
...
proof. by elim: s1 => //= x s1 IHs; rewrite IHs; smt(hasP). qed.
...
proof. by elim: s1; smt(hasP). qed.

lemma nth_find z0 p (s : 'a list): has p s => p (nth z0 s (find p s)).
proof. by elim: s; smt(find_ge0). qed.
...
proof.
  move=> n_ge0 i_ge0; case: (n < size s)=> [lt_n_s|]; last smt(nth_out size_drop).
...
proof.
  move=> n_ge0 i_ge0; case: (n < size s) => [lt_n_s|]; last smt(take_oversize).
  by rewrite -{2}(cat_take_drop n s) nth_cat size_take //; smt().
qed.
...
proof. smt(take_oversize drop_oversize). qed.
...
proof. smt(size_rot rot_le0 rot_oversize size_drop rot_size_cat cat_take_drop). qed.
...
proof.
  ...
  rewrite !cnt_a' (eq_cnt1 x) // (IHi a') //.
  smt(has_pred1 has_count size_ge0).
qed.
...
proof. elim: s; smt(rev_cons rev_rcons). qed.
...
proof. by elim: s i; smt(index_cons mem_nth). qed.
...
smt(map_cons rev_cons map_rcons).
qed.
...
proof.
  ...
  by move=> i Hi; rewrite nth_mkseq //; smt(size_ge0).
qed.
